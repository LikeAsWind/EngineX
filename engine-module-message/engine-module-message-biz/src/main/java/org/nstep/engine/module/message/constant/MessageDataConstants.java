...
public class MessageDataConstants {
     ...
     * 手机号正则表达式
     * <p>
     * 用于验证手机号格式是否符合中国大陆的号码规范。
     * 匹配规则：
     * - 以 13、14、15、16、17、18、19 开头的号码段
     * - 后续跟随 8 位数字
     * - 支持常见的运营商号段，例如移动、联通、电信
     ...
     * 邮箱正则表达式
     * <p>
     * 用于验证电子邮件地址的格式是否符合标准规范。
     * 匹配规则：
     * - 本地部分：允许字母、数字、点（.）、下划线（_）、百分号（%）、加号（+）和连字符（-）
     * - 必须包含 @ 符号分隔本地部分和域名部分
     * - 域名部分：允许字母、数字和连字符，必须以字母结尾
     * - 顶级域名（TLD）：长度至少为 2 的字母
     * 示例：example@domain.com、user.name+tag@sub.domain.org
     */
